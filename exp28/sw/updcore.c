...
void copyfromfile2spi (BYTE handle, BYTE fpga);
...
BYTE commandlinemode (char *p)
{
  ...
  if (fpga == 1)
    fname = (char *)"/bin/zxspa35.bit";
  else if (fpga == 2)
    fname = (char *)"/bin/zxspa100.bit";
  else
    fname = (char *)"/bin/zxspa200.bit";
  ...
  if (handle==0xff)
  {
    ...
    return errno;
      ...
      // 01234567890123456789012345678901
  puts ("ZXSPECTRUM CORE UPDATER FOR ZX3\x0D"
        "-------------------------------\x0D"
        "\x0D"
...
{
        // 01234567890123456789012345678901
    puts ("UPDCORE [options]\x0D\x0D"
          "Update ZX Spectrum core in flash"
          "memory from BIT file.\x0D"
          "Options:\x0D"
          "-a35t  : FPGA is Artix 7-35\x0D"
          "-a100t : FPGA is Artix 7-100\x0D"
          "-a200t : FPGA is Artix 7-200\x0D\x0D"
          "The BIT file must reside in the\x0D"
...
void copyfromfile2spi (BYTE handle, BYTE fpga)
{
    ...
    pagina = 4096;  // primera pagina de la flash donde se escribe el core de Spectrum
    
    read (handle, buffer, 0x79);  // leemos (y saltamos) la cabecera del BIT
    if (fpga == 1 && buffer[0x4F] != '3'|| fpga == 2 && buffer[0x4F] != '1'|| fpga == 3 && buffer[0x4F] != '2')
    {
      puts("\x0D"
           "Error! FPGA and bitstream mismatch!\x0D");
      return;
    }
